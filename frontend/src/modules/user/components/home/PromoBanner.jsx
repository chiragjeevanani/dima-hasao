...
export const PromoBanner = () => {
  ...
  return (
    <div className="relative w-auto mx-2.5 h-[84px] rounded-2xl overflow-hidden shadow-xs border border-orange-200/40" data-purpose="promo-banner">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-r from-amber-500/25 via-transparent to-transparent pointer-events-none"></div>

      {/* Left Typography & CTA */}
      <div className="absolute inset-y-0 left-0 p-2.5 flex flex-col justify-center w-3/5 z-10">
        <h3 className="text-gray-900 font-extrabold text-[10px] sm:text-[10.5px] leading-tight mb-1 drop-shadow-xs tracking-tight">
          DISCOVER THE<br />
          UNTOLD BEAUTY OF<br />
          <span className="text-[#084524]">DIMA HASAO</span>
        </h3>
        <motion.button
          whileHover={{ scale: 1.03 }}
          whileTap={{ scale: 0.95 }}
          onClick={() => navigate('/places')}
          className="bg-[#084524] hover:bg-emerald-800 text-white text-[7.5px] font-bold py-0.5 px-2 rounded-full w-max flex items-center gap-1 shadow-xs cursor-pointer border border-white/20"
        >
          <span>Plan Your Trip Now</span>
          <i className="fa-solid fa-arrow-right text-[6.5px]"></i>
        </motion.button>
      </div>

      {/* Decorative Circular Badges Overlapping on Right */}
      <div className="absolute right-2 top-1/2 -translate-y-1/2 flex -space-x-2.5 z-10 pointer-events-none">
        <div className="w-10 h-10 rounded-full border-2 border-white overflow-hidden shadow-xs bg-white">
          <img
            alt="Waterfall"
            className="w-full h-full object-cover"
        ...
        <div className="w-11 h-11 rounded-full border-2 border-white overflow-hidden shadow-md z-10 bg-white">
          <img
            alt="Traditional Hut"
            className="w-full h-full object-cover"
